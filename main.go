...
func main() {
	port := "502"
	// Allow overriding port via first argument (useful when 502 requires root)
	if len(os.Args) > 1 {
		port = os.Args[1]
	}
	...
	if err != nil {
		...
		fmt.Fprintf(os.Stderr, "         Utilisez: sudo ./serveur-modbus-tcp  ou  ./serveur-modbus-tcp 5020\n")
		os.Exit(1)
	}
